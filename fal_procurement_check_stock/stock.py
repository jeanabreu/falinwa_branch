...
class stock_move(models.Model):
    ...
    def _prepare_procurement_from_move(self, move):
        ...
        if move.rule_id.route_id.fal_check_stock or True in [(x.fal_check_stock) for x in move.route_ids]:
            product_virtual_available = product_obj.browse([move.product_id.id]).with_context(location= move.location_id.id)._product_available()[move.product_id.id]['virtual_available']
