...
class account_invoice(orm.Model):
    ...
    def _amount_ballance_cny(self, cr, uid, ids, name, args, context=None):
        ...
        for invoice in self.browse(cr, uid, ids, context=context):
            ...
            rate_ids = currency_obj.search(cr, uid,[('name', '=', 'CNY'),('company_id','=',invoice.company_id.id)] , context=ctx, limit=1)
            cur = invoice.currency_id
            if invoice.move_id:
    ...
    _columns = {
        ...
        'amount_ballance_cny': fields.function(_amount_ballance_cny, digits_compute=dp.get_precision('Account'), string='Balance (CNY)',
            store={
            ...
            }, help="The balance amount in CNY.")
    }
