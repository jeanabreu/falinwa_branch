...
class stock_invoice_onshipping(orm.TransientModel):
    ...
    def view_init(self, cr, uid, fields_list, context=None):
        ...
        if len(active_ids) == 1 and count:
            raise orm.except_orm(_('Warning!'), _('This picking list does not require invoicing.'))
        if len(active_ids) == count:
            raise orm.except_orm(_('Warning!'), _('None of these picking lists require invoicing.'))
        return res
    ...
    def open_invoice(self, cr, uid, ids, context=None):
        ...
        inv_type = context.get('inv_type', False) or res.get('inv_type', False)
        action_model = False
        action = {}
        if not invoice_ids:
            raise orm.except_orm(_('Error!'), _('Please create Invoices.'))
    ...
    def create_invoice(self, cr, uid, ids, context=None):
        ...
        if not inv_type:
            if active_picking.move_lines:
                src_usage = active_picking.move_lines[0].location_id.usage
                dest_usage = active_picking.move_lines[0].location_dest_id.usage
            if active_picking.type == 'out' and dest_usage == 'supplier':
                inv_type = 'in_refund'
            elif active_picking.type == 'out' and dest_usage == 'customer':
                inv_type = 'out_invoice'
            elif active_picking.type == 'in' and src_usage == 'supplier':
                inv_type = 'in_invoice'
            elif active_picking.type == 'in' and src_usage == 'customer':
                inv_type = 'out_refund'
            else:
                inv_type = 'out_invoice'
...
#end of stock_invoice_onshipping()
