...
class purchase_order(orm.Model):
    ...
    def _total_uninvoice_cny(self, cr, uid, ids, field_name, arg, context=None):
        cur_obj = self.pool.get('res.currency')
        res = {}
        ctx = context.copy()
        invoice_obj = self.pool.get('account.invoice')
    ...
    _columns = {
        ...
        'total_uninvoice_cny': fields.function(_total_uninvoice_cny, digits_compute=dp.get_precision('Account'), string='Total Uninvoice (CNY)',
            store={
                'purchase.order' : (lambda self, cr, uid, ids, c={}: ids, None, 20),
                'account.invoice' : (_get_invoice_ids_fal, ['invoice_line','tax_line','state','currency_id'], 20),
            }, help="The total uninvoice in CNY."),
    }
