...
class account_voucher(orm.Model):
    ...
    _columns = {
        'fal_due_date' : fields.date('Due Date', states={'draft':[('readonly',False)]}),
        'fal_bank_guarantee_paid_date' : fields.date('Bank Guarantee Paid Date', readonly=1),
        'fal_payment_type': fields.selection([('Normal','Normal'),('Bank Guarantee','Bank Guarantee')], 'Type', required=True, states={'draft':[('readonly',False)]}),
        'fal_is_bank_guarantee_paid' : fields.boolean('Bank Guarantee Paid', readonly=1),
        'fal_is_bank_guarantee_payment' : fields.boolean('Bank Guarantee Payment'),
    }    

    _defaults = {
        'fal_payment_type' : 'Normal',
    }
    ...
    def recompute_voucher_lines(self, cr, uid, ids, partner_id, journal_id, price, currency_id, ttype, date, context=None):
        ...
        if not context.get('move_line_ids', False):
            ids = move_line_pool.search(cr, uid, [('state','=','valid'), ('account_id.type', 'in', ['payable','receivable']), ('account_id.fal_bank_product', '=', False), ('reconcile_id', '=', False), ('partner_id', '=', partner_id)], context=context)
        else:
    ...
    def recompute_bank_guarantee_voucher_lines(self, cr, uid, ids, partner_id, journal_id, price, currency_id, ttype, date, context=None):
        """
        Returns a dict that contains new values and context
        ...
        if not context.get('move_line_ids', False):
            ids = move_line_pool.search(cr, uid, [('state','=','valid'), ('account_id.type', 'in', ['payable','receivable']), ('account_id.fal_bank_product', '=', True), ('reconcile_id', '=', False), ('partner_id', '=', partner_id)], context=context)
        else:
            ids = context['move_line_ids']
        invoice_id = context.get('invoice_id', False)
        company_currency = journal.company_id.currency_id.id
    ...
    def onchange_bank_guarantee_partner_id(self, cr, uid, ids, partner_id, journal_id, amount, currency_id, ttype, date, context=None):
        if not journal_id:
            return {}
        if context is None:
            context = {}
        ...
        vals = self.recompute_bank_guarantee_voucher_lines(cr, uid, ids, partner_id, journal_id, amount, currency_id, ttype, date, context=ctx)
    ...
    def onchange_journal_bank_guarantee(self, cr, uid, ids, journal_id, line_ids, tax_id, partner_id, date, amount, ttype, company_id, context=None):
        if context is None:
            context = {}
        if not journal_id:
            return False
        ...
        if context.get('payment_expected_currency') and currency_id != context.get('payment_expected_currency'):
            ...
            amount = 0
        if partner_id:
            res = self.onchange_bank_guarantee_partner_id(cr, uid, ids, partner_id, journal_id, amount, currency_id, ttype, date, context)
            for key in res.keys():
                vals[key].update(res[key])
        return vals

    def onchange_amount_bank_guarantee(self, cr, uid, ids, amount, rate, partner_id, journal_id, currency_id, ttype, date, payment_rate_currency_id, company_id, context=None):
        if context is None:
            context = {}
        ctx = context.copy()
        ctx.update({'date': date})
        ...
        res = self.recompute_bank_guarantee_voucher_lines(cr, uid, ids, partner_id, journal_id, amount, currency_id, ttype, date, context=ctx)
    ...
    def proforma_voucher(self, cr, uid, ids, context=None):
        ...
        for voucher in self.browse(cr, uid, ids):
            if voucher.fal_is_bank_guarantee_payment:
                for line in voucher.line_ids:
                    if line.reconcile:
                        voucher_to_paid_ids = self.search(cr, uid, [('move_id','=', line.move_line_id.move_id.id)])
                        if voucher_to_paid_ids:
                            self.write(cr, uid, [voucher_to_paid_ids[0]], {
                                'fal_is_bank_guarantee_paid': 1,
                                'fal_bank_guarantee_paid_date': voucher.date,
                            })
        return res
        ...
        for voucher in self.browse(cr, uid, ids):
            if voucher.fal_is_bank_guarantee_payment:
                for line in voucher.line_ids:
                    ...
                    if voucher_to_paid_ids:
                        self.write(cr, uid, [voucher_to_paid_ids[0]], {
                            'fal_is_bank_guarantee_paid': 0,
                            'fal_bank_guarantee_paid_date': False,
                        })
        return res
...
class account_account(orm.Model):
    ...
    _columns = {
        'fal_bank_product': fields.boolean('Bank Product'),
    } 
